...
def get_model_embeddings_from_loader(
    model, loader, return_labels=False, do_progress_bar=True, device="cuda"
):
    """ """
    ...
    with torch.no_grad():
        iterator = (
            tqdm.tqdm(loader, position=0, leave=True) if do_progress_bar else loader
        )
...
def get_model_embeddings_from_tensors(
    model,
    tensor,
    tensor_labels=None,
    batch_size=64,
    ...
    return get_model_embeddings_from_loader(
        model,
        loader,
