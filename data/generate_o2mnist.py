...
transform = T.Compose((
    T.ToTensor(), T.Pad(add_padding), 
    T.Resize(32*3),
    T.RandomRotation(360), T.RandomVerticalFlip(0.5),
    T.Resize(32),
))

def get_o2_mnist(train=True):
    dset = torchvision.datasets.MNIST(root=data_dir, download=True, transform=transform, train=train)
