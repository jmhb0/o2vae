...
def loss_reconstruction_fourier_batch(x, y, recon_loss_type="bce", mask=None):
    ...
    if recon_loss_type == "bce":
        ...
        raise ValueError(f"loss func not supported")

    # get the flip
    yf = T_f.vflip(y)

    # take the polar transform
    PolarTransform = registration.PolarTranformBatch(
        image_shape=x.shape[-2:], scaling="linear"
    )
    x_polar, y_polar, yf_polar = (
        PolarTransform.warp_batch(x),
        PolarTransform.warp_batch(y),
        PolarTransform.warp_batch(yf),
    )

    # compute the rotation
    shifts_r, error_r, phasediff_r = registration.phase_correlation_2d_batch(
        x_polar, y_polar
    )
    angle_r = -shifts_r[:, 0]
    shifts_f, error_f, phasediff_f = registration.phase_correlation_2d_batch(
        x_polar, yf_polar
    )
    angle_f = -shifts_f[:, 0]
    ...
    M_r = tgm.get_rotation_matrix2d(center=center, angle=-angle_r, scale=scale).to(
        x.device
    )
    M_f = tgm.get_rotation_matrix2d(center=center, angle=-angle_f, scale=scale).to(
        x.device
    )
    grid_r = torch.nn.functional.affine_grid(M_r, size=x.shape, align_corners=True).to(
        x.device
    )
    grid_f = torch.nn.functional.affine_grid(M_f, size=x.shape, align_corners=True).to(
        x.device
    )
...
def rotated_flipped_xs(x, rot_steps, trans=None, do_flip=True, upsample=0):
    ...
    x_expanded = (
        x.unsqueeze(0)
        .expand(n_angles, *x.shape)
        .contiguous()
        .view(n_angles * bs, c, h, w)
    )
...
def loss_smallest_over_rotations(
    x,
    y,
    rot_steps=1,
    do_flip=True,
    ...
    bs, c, m, n = x.shape
    # x annd y may have different shapes if recon_loss_type=='ce' (doing multiclass classification)
    (_, c_y, _, _) = y.shape

    # use rot_steps val defined in __init__
    xs = rotated_flipped_xs(x, rot_steps=rot_steps, do_flip=do_flip)
    n_rots = len(xs)

    # duplicate the y dimension for each rotation
    y = y.view(1, bs, c_y, m, n).expand(
        n_rots, bs, c_y, m, n
    )  # duplicate along the first dimension
