...
activations = dict(
    tanh=nn.Tanh, lrelu=nn.LeakyReLU, relu=nn.ReLU, elu=nn.ELU, none=nn.Identity
)
...
class CnnEncoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.cnn_layers = make_cnn_layers(
            in_channels=n_channels,
            hidden_dims=cnn_dims,
            kernel_size=kernel_size,
            stride=stride,
            padding=padding,
            activation=activation,
            do_norm_layer=do_norm_layer,
        )
        # expect 1*1 feature maps
        self.fc_layer = nn.Linear(1 * 1 * cnn_dims[-1], n_classes)
