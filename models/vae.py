...
class VAE(torch.nn.Module):
    ...
    def prior_setup(self, prior_kwargs):
        ...
        # mixture of gaussias, with learnable gaussian params, but fixed uniform weigths, k
            ...
            self.z_pre = torch.nn.Parameter(torch.randn(1, 2 * self.k, self.zdim)
                                        / scaler )
    ...
    def embed_data(self, x):
        ...
        one batch at a time"""
        if len(x)>256:
            msg = "This function is for batches only. If you need embeddings for more"\
    ...
    def loss_recon(self, x, y):
        ...
        """
        if self.mask: self.mask=self.mask.to(x.device)
    ...
    def loss(self, x, y, mu, logvar, ):
        ...
        return dict(loss=loss, loss_recon=loss_recon, loss_kl=loss_kl, beta=self.loss_kwargs['beta'])
    ...
    def decode_img(self, z):
        ...
        """
        y = self.p_net(z)
        if self.do_sigmoid: y = torch.sigmoid(y)
