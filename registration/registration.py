...
class PolarTranformBatch():
    ...
    """
    def __init__(self, image_shape, center=None, radius=None, output_shape=None, scaling="linear", 
                order=1, cval=0, mode_grid_sample_padding="zeros", mode_grid_sample_interpolate="bilinear",
        ...
        self.warp_args = {'k_angle': k_angle, 'k_radius': k_radius, 'center': center.numpy()}
        ...
        self.coords = torch.Tensor(skimage.transform._warps.warp_coords(coord_map, output_shape))
        ...
        self.coords_torch_format = self.coords.clone().moveaxis(0,2) # move to shape (2,Y,X) to (Y,X,2)
        min0, min1, max0, max1 = 0, 0, image_shape[0], image_shape[1]
        self.coords_torch_format[:,:,0] = (self.coords_torch_format[:,:,0]-min0) / (max0-min0) * 2 - 1
        self.coords_torch_format[:,:,1] = (self.coords_torch_format[:,:,1]-min1) / (max1-min1) * 2 - 1
        self.coords_torch_format = self.coords_torch_format[:,:,[1,0]] # grid_sample has the order reversed
    ...
    def warp_single_ndimage(self, x):
        ...
        """
        assert x.ndim==2
        if type(x) is torch.Tensor: x=x.cpu().numpy()
        warped = ndi.map_coordinates(x, self.coords, prefilter=self.prefilter,
                         mode=self.ndi_mode, order=self.order, cval=self.cval)
    ...
    def warp_batch(self, x):
        ...
        assert x.ndim==4
        batch_size=len(x)
        coords_batch=self.coords_torch_format.unsqueeze(0).expand(batch_size, *self.coords_torch_format.shape).to(x.device)
        warped_batch=torch.nn.functional.grid_sample(x, coords_batch, align_corners=True, 
                                padding_mode=self.mode_grid_sample_padding, mode=self.mode_grid_sample_interpolate)
        return warped_batch 
    ...
    def _verify_warping_funcs_are_the_same(self, x=None, do_plot=1):
        ...
        x_batch = x.unsqueeze(0).unsqueeze(0).expand(1,1,*x.shape) # put the test image into a batch
...
def phase_correlation_2d_batch(x,y, normalization=None, upsample_factor=1, return_error=1, space='real'):
    """
    ...
    shifts[shifts>midpoints] -= torch.Tensor([[ydim,xdim]]).to(device).float().expand((bs,2))[shifts>midpoints]
